//this configuration decodes the encrypted texts at 
//http://forum.arduino.cc/index.php?topic=52283.0
//after decoding the first text, issue the command !GMA to reset the wheels to the starting position of GMA again
//then the reply text can be decoded
...
void initEnigma()
{
  ...
  EnigmaData.WHEELTYPE[3] = 3;     // LEFT ROTOR
  EnigmaData.WHEELTYPE[4] = 0;     // ADDITIONAL WHEEL (M4 only)
  EnigmaData.WHEELTYPE[5] = 14;    // REFLECTOR: UKW

  //Wheel Positions
  EnigmaData.WHEELPOS[0] = 0;      // LEFTMOST LETTER ON M4
  EnigmaData.WHEELPOS[1] = 'G';    // LEFTMOST LETTER ON M3
  EnigmaData.WHEELPOS[2] = 'M';    // MIDDLE LETTER
  EnigmaData.WHEELPOS[3] = 'A';    // RIGHTMOST LETTER

  //Ring Settings
  EnigmaData.ROTORPOS[0] = 0;           // LEFTMOST ROTOR SETTING ON M4
  EnigmaData.ROTORPOS[1] = 'E'-64;      // LEFTMOST ROTOR SETTING ON M3
  EnigmaData.ROTORPOS[2] = 'N'-64;      // MIDDLE ROTOR SETTING
  EnigmaData.ROTORPOS[3] = 'I'-64;      // RIGHTMOST ROTOR SETTING
